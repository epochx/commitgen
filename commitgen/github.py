...
class GitHub(object):
    ...
    def _http(self, _method, _path, **kw):
        ...
        try:
            response = opener.open(request, timeout=TIMEOUT)
            _type, _more = self._process_resp(response.headers)
            if _type == "json":
                return _parse_json(response.read().decode('utf-8')), _more
            if _type == "diff":
                return response.read().decode('utf-8', 'ignore'), _more
        except HTTPError as e:
    ...
    def _process_resp(self, headers):
        ...
        if headers:
            for k in headers:
                ...
                # adding support for diff files
                elif h == 'content-type':
                    if headers[k].startswith('application/json'):
                        _type = 'json'
                    elif headers[k].startswith('application/vnd.github.v3.diff'):
                        _type = 'diff'
                # adding support for pagination
                if h == 'link':
                    if 'rel="next"' in headers[k]:
                        _more = True
        return _type, _more
